...
def inference(network,video_file):
    print("Entering inference function")
    image_seq = deque([],8)
    cap = cv2.VideoCapture(video_file)
    counter = 0 
    stat = 'safe'
    while (cap.isOpened()):
        # print('Entering inference while loop')
        ret, frame = cap.read()
        if ret:
            ...
            if counter%2 == 0:
                if len(image_seq)==8:
                    ...
                    print(f'Status of frame {counter}: {stat}')
...
if  __name__ == "__main__":
    ...
    if mode == 'train':
        ...
        network.load_weights(os.path.join(model_save_folder, 'model_weights_052.ckpt'))
        inference(network, os.path.join(base_folder, 'files', 'out2.avi'))
